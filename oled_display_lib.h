...
void     Disp_PutText(char *str);             // Display text string at (x, y)
...
void     Disp_PutDecimalWord(uint16_t val, uint8_t fieldSize);  // Show uint16_t in decimal
void     Disp_BlockFill(uint16_t w, uint16_t h);   // Fill area, w x h pixels, at cursor (x, y)
