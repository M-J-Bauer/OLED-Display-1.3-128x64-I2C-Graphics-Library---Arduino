...
{
    I2C_MasterStart(SH1106_I2C_ADDRESS);
    I2C_MasterSend(SH1106_MESSAGETYPE_COMMAND);
    I2C_MasterSend(cmd);
    I2C_MasterStop();
}
...
{
    I2C_MasterStart(SH1106_I2C_ADDRESS);
    I2C_MasterSend(SH1106_MESSAGETYPE_DATA);
    I2C_MasterSend(data);
    I2C_MasterStop();
}
...
void  SH1106_Init(void)
{
    ...
    SH1106_WriteCommand(0x80);                                 // init value = 0x80
    ...
    for (page = firstPage;  page <= lastPage;  page++)
    {
        ...
        for (col = firstCol;  col <= lastCol && col < 8;  col++)
        {
            ...
            for (x_coord = 0;  x_coord < 16;  x_coord++)
            {
                ...
                I2C_MasterSend(pixels);  // segment address auto increments
            }
            I2C_MasterStop();
        }
    }
